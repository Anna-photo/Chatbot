...
window.registerKnowledgeModule({ language: 'korean', category: 'general', run: function(lowerMessage, userMessage, lang) {
  if (lowerMessage.includes("salut")) {
            return "Salut! Comment ça va ?";
          }
    
          // For "안녕" and "안녕하세요" (hello)
  else if (lowerMessage.includes("안녕") || lowerMessage.includes("안녕하세요")) {
  ...
  else if (lowerMessage.includes("anthem") && lowerMessage.includes("north korea")) {
  ...
  else if (lowerMessage.includes("anthem") && lowerMessage.includes("south korea")) {
        return "<h2 style='display: flex; align-items: center; justify-content: center;'>" +
